...
void help() {
  ...
  printf("\n");
  printf("Lumiere : \n");
  printf("    alt+fleches ou pageup,pagedown : bouge la lumiere\n");
  printf("\n");
  printf("Touches : \n");
...
void GLColor(const float r, const float g, const float b) {
  ...
  // when light is disable
  glColor3f(r, g, b);
}

//! A appeller juste avant la boucle principale (Main Loop) et apr�s la creation
//! de a fenetre
GLvoid GLInit(GLsizei Width, GLsizei Height) {
  ...
  // Indique � GL de calculer la couleur suivant la formule
  // couleur_du_buffer = alpha * couleur_polygone +
  // (1-alpha)*couleur_du_buffer_de_fond
  glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
  glDisable(GL_BLEND);

  // Indique � GL d'eliminer les fragments du polygones qui sont transparents
...
void processMouse(int button, int state, int x, int y) {
  ...
  if (state == GLUT_DOWN)
  //((state == GLUT_DOWN)) && (specialKey == GLUT_ACTIVE_ALT))
  {
